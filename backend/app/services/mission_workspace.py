...
class MissionWorkspaceService:
    ...
    def get(self, mission_id: UUID | str) -> MissionWorkspace:
        ...
        if mission.status == "running" and (
            started is None or started < mission.updated_at
        ):
        ...
        if (
            ...
            and terminal
            and terminal.event_type == "workflow_completed"
        ):
            summary = MissionRunSummary.model_validate(terminal.metadata_json)
            opportunities = [
                item
                for item in self.opportunities.list_for_mission(mission_id)
                if started and item.created_at >= started
            ]
            if (
                summary.decision
                and summary.decision.recommendation == "proceed_with_poc"
                and any(event.event_type == "action_plan_created" for event in current)
            ):
        ...
        return MissionWorkspace(
            ...
            error=(
                failure.message
                if failure
                else "Workflow failed before producing a result."
            )
